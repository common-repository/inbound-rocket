...
class WPContactsAdmin extends WPInboundRocketAdmin {
     ...
     */
    public function power_up_setup_callback ()
    {
        $this->inboundrocket_contacts_page();
    }
    ...
    function inboundrocket_contacts_page ()
    {
        ...
        {
            $lead_id = ( isset($_GET['lead']) ? intval($_GET['lead']) : FALSE );
        ...
        echo '<div id="inboundrocket" class="wrap '. ( $wp_version < 3.8 && !is_plugin_active('mp6/mp6.php')  ? 'pre-mp6' : ''). '">';
            ...
            if ( $this->action !== 'view' ) {
                ...
                $this->inboundrocket_render_contact_detail( intval($_GET['lead']) );
            }
	        ...
	        <div class="ir-frame">
		        <div class="header">
					<nav role="navigation" class="header-nav drawer-nav nav-horizontal">
						<ul class="main-nav">
							<li class="inboundrocket-logo"><a href="<?=admin_url('admin.php?page=inboundrocket_stats');?>" title="<?php _e( 'Inbound Rocket Stats', 'inbound-rocket' );?>"><span>Inbound Rocket</span></a></li>
						</ul>
					</nav>
				</div><!-- header -->
...
<?php
        if ( isset($_GET['stats_dashboard']) )
            echo '<a href="' . admin_url('admin.php?page=inboundrocket_stats') . '">&larr; '. __( 'Stats Dashboard', 'inbound-rocket' ) .'</a>';
        else
        {
            if ( isset($_GET['redirect_to']) )
            {
                if ( strstr($_GET['redirect_to'], 'contact_type') )
                {
                    ...
                    if ( isset($url_vars['contact_type']) && $url_vars['contact_type'] )
                        echo '<a href="' . esc_url($_GET['redirect_to']) . '">&larr; ' . __( 'All', 'inbound-rocket' ).''. ucwords($url_vars['contact_type']) . '</a>';
                    else
                        echo '<a href="' . esc_url($_GET['redirect_to']) . '">&larr; ' . __( 'All Contacts', 'inbound-rocket' ) .'</a>';
                }
                else
                    echo '<a href="' . esc_url($_GET['redirect_to']) . '">&larr; ' . __( 'All Contacts', 'inbound-rocket' ) .'</a>';
                ...
                echo '<a href="' . admin_url('/admin.php?page=inboundrocket_contacts') . '">&larr; ' . __( 'All Contacts', 'inbound-rocket' ) .'</a>';
        }
		...
		echo '<h2 class="' . $css_class . '">' . __( 'Contact info of:', 'inbound-rocket' ) .' ';
        if (!empty($lead_full_name)) { echo esc_html( $lead_full_name ); } elseif (!empty($contact->lead_first_name)) { echo esc_html( $contact->lead_first_name )." "; } elseif (!empty($contact->lead_last_name)) { echo esc_html( $contact->lead_last_name )." "; } else { _e( 'No name provided', 'inbound-rocket' );}
        // echo !empty($lead_full_name) ? $lead_full_name : !empty($lead_firstname) ? esc_html($lead_firstname)." " : !empty($lead_lastname) ? esc_html($lead_lastname)." " : __( 'No name provided', 'inbound-rocket' );
                ...
                echo '<div class="contact-lists">';
                 if(isset($ir_contact->history->lead_lists)):
                    ...
                    <div id="edit-contact-lead-lists" style="display:none;">
                        <h2><?php _e('Edit Lists','inbound-rocket'); ?> - <?php echo $ir_contact->history->lead->lead_email; ?></h2>
                        <form id="edit_lists" action="#" target="_parent" method="POST">
                            ...
                            <p class="submit">
                                <input type="submit" name="submit" id="submit" class="button button-primary" value="<?php _e( 'Save To List(s)', 'inbound-rocket' ); ?>">
                            </p>
                        </form>
                    </div>

                    <a class="thickbox contact-edit-lists" href="#TB_inline?width=300&height=450&inlineId=edit-contact-lead-lists"><?php _e( 'edit lead lists', 'inbound-rocket' ); ?></a>
		...
		echo	'<div id="col-right" class="metabox-holder">';
		echo		'<div class="col-header">'. __('Lead Activity', 'inbound-rocket') .'</div>';
		...
		echo			'<ul class="sessions">';
                    ...
                    foreach ( $sessions as &$session )
                    {
                        ...
                        foreach ( $events as &$event )
                        {
                            if ( $event['event_type'] == 'pageview' )
                            {
                                ...
                                if ( isset($pageview['event_date']) && $pageview['event_date'] == $first_event['event_date'] )
                                {
                                    echo '<li class="event source">';
                                        ...
                                        echo '<div class="event-content">';
                                            echo '<p class="event-title">' . __( 'Traffic Source', 'inbound-rocket' ) .': ' . ( $pageview['pageview_source'] ? '<a href="' . $pageview['pageview_source'] . '">' . inboundrocket_strip_params_from_url($pageview['pageview_source']).'</a>' : 'Direct' ) . '</p>';
                                ...
                                echo '<li class="event form-submission">';
                                    ...
                                    echo '<div class="event-content">';
                                        echo '<p class="event-title">';
                                            echo '' . __( 'Filled out Form', 'inbound-rocket' ) .' (' . $event['form_name'] . ') ' . __( 'on page', 'inbound-rocket' ) .' <a href="' . $submission['form_page_url'] . '">' . $submission['form_page_title']  . '</a>';
                                            if ( isset($event['form_tags'][0]['tag_slug']) )
                                            {
                                                echo ' ' . __( 'and tagged as', 'inbound-rocket' ) .' ';
                            ...
                            else if ($event['event_type'] == 'text-share')
                            {
	                            ...
	                            if ($share['share_type']=='ss-twitter-text') { $type = __( 'Twitter (using Selection Sharer power-up)', 'inbound-rocket' ); }
	                            elseif ($share['share_type']=='ss-facebook-text') { $type = __( 'Facebook (using Selection Sharer power-up)', 'inbound-rocket' ); }
	                            elseif ($share['share_type']=='ss-linkedin-text') { $type = __( 'LinkedIn (using Selection Sharer power-up)', 'inbound-rocket' ); }
	                            elseif ($share['share_type']=='ss-email-text') { $type = __( 'Email (using Selection Sharer power-up)', 'inbound-rocket' ); }
	                            elseif ($share['share_type']=='click-to-tweet') { $type = __( 'Twitter (using Click-To-Tweet power-up)', 'inbound-rocket' ); }
	                            
	                           	echo '<li class="event text-share">';
									echo '<div class="event-time">'.date('g:ia', strtotime($share['event_date'])).'</div>';
									echo '<div class="event-content">';
									echo	'<p class="event-title">' . __( 'Shared a text snippet from', 'inbound-rocket' ) .' <a href="'.$url.'">'.$title.'</a></p>';
										echo '<ul class="event-detail fields">';
											echo '<li class="field">';
												echo '<label class="field-label">' . __( 'Text shared', 'inbound-rocket' ) .':</label>';
												echo '<p class="field-value">'.$share['share'].'</p>';
											echo '</li>';
											echo '<li class="field">';
												echo '<label class="field-label">' . __( 'Shared to', 'inbound-rocket' ) .':</label>';
                            ...
                            else if ($event['event_type'] == 'image-share')
                            {
	                            ...
	                            if ($share['share_type']=='is-twitter-image') { $type = __( 'Twitter (using Image Sharer power-up)', 'inbound-rocket' ); }
	                            elseif ($share['share_type']=='is-facebook-image') { $type = __( 'Facebook (using Image Sharer power-up)', 'inbound-rocket' ); }
	                            elseif ($share['share_type']=='is-pinterest-image') { $type = __( 'Pinterest (using Image Sharer power-up)', 'inbound-rocket' ); }
	                            
	                            echo '<li class="event image-share">';
									echo '<div class="event-time">'.date('g:ia', strtotime($share['share_date'])).'</div>';
									echo '<div class="event-content">';
										echo '<p class="event-title">' . __( 'Shared an image from', 'inbound-rocket' ) .' <a href="'.$url.'">'.$title.'</a></p>';
										echo '<ul class="event-detail fields">';
											echo '<li class="field">';
												echo '<label class="field-label">' . __( 'Image shared', 'inbound-rocket' ) .'</label>';
			...
			echo '<div id="col-left" class="metabox-holder">';
				echo '<div class="col-header">'. __('Lead Profile', 'inbound-rocket') .'</div>';
					echo '<div class="inboundrocket-meta-section">';
						echo '<table class="inboundrocket-meta-table">';
							echo '<tbody>';
								echo '<tr>';
									echo '<td>'; 
										echo '<img class="contact-header-avatar inboundrocket-dynamic-avatar_'. esc_attr($lead_id) .'" src="https://www.gravatar.com/avatar/' . $gravatar_hash .'" alt="Avatar" />';
									echo '</td>';
									echo '<td>';
										echo '<table>';
											echo '<tr>';
												echo '<td><strong>' . __( 'Name', 'inbound-rocket' ) .':</strong></td>';
												echo '<td style="padding-left:10px;">';
												if (!empty($lead_full_name)) { echo esc_html( $lead_full_name ); } elseif (!empty($lead_firstname)) { echo esc_html( $lead_firstname )." "; } elseif (!empty($lead_lastname)) { echo esc_html( $lead_lastname )." "; } else { _e( 'No name provided', 'inbound-rocket' );}
												// echo !empty($lead_full_name) ? $lead_full_name : !empty($lead_firstname) ? esc_html($lead_firstname) : !empty($lead_lastname) ? esc_html($lead_lastname) : __( 'No name provided', 'inbound-rocket' );
												echo '</td>';
											echo '</tr>';
											echo '<tr>';
												echo '<td><strong>' . __( 'Email', 'inbound-rocket' ) .':</strong></td>';
												echo '<td style="padding-left:10px;"><a href="mailto:'. esc_html($lead_email) .'" target="_blank">'.esc_html($lead_email).'</a></td>';
											echo '</tr>';
											echo '<tr>';
												echo '<td><strong>' . __( 'Original source', 'inbound-rocket' ) .':</strong></td>';
								...
								echo '<tr>';
									...
									echo '<td><strong>' . __( 'Location', 'inbound-rocket' ) .':</strong><br /><span class="citystate">' . esc_html($lead_city) . ', ' .esc_html($lead_state). '</span> ' . esc_html($lead_country) . '</td>';
									$map_link_location = urlencode(esc_html($lead_city).',+'.esc_html($lead_state));
									echo '<td style="text-align:right;"><a target="_blank" href="https://www.google.com/maps/place/'.$map_link_location.'">' . __( 'View Larger Map', 'inbound-rocket' ) .'</a></td>';
								echo '</tr>';
								echo '<tr>';
									echo '<td colspan="2"><img class="contact-map" alt="Map" src="https://maps.googleapis.com/maps/api/staticmap?center=' . esc_attr(urlencode($lead_loc)) . '&zoom=13&size=660x175&maptype=roadmap&markers=color:red%7C' . esc_attr(urlencode($lead_loc)) . '" /></td>';
								echo '</tr>';
							echo '</tbody>';
						echo '</table>';
					echo '</div>';
				...
				echo '<div class="inboundrocket-meta-section">';
					echo '<h4 class="inboundrocket-meta-header inboundrocket-premium-tag">' . __( 'Personal Info', 'inbound-rocket' ) .'</h4>' . __( 'More background information about the contact, coming soon for premium users.', 'inbound-rocket' );
					echo '<table class="inboundrocket-meta-table">';
						echo '<tbody>';
							echo '<tr><td>&nbsp;</td></tr>';
						echo '</tbody>';
				...
				echo '<div class="inboundrocket-meta-section">';
					echo '<h4 class="inboundrocket-meta-header inboundrocket-premium-tag">' . __( 'Company Info', 'inbound-rocket' ) .'</h4>';
					// echo '<p><b>About '. esc_html($company_name) .'</b></p>'; 
					?>
					<table class="inboundrocket-meta-table">
						<tbody>
							<tr><td><?php _e('In our upcoming release for premium users we will display all relevant company info.', 'inbound-rocket'); ?></td></tr>
						</tbody>
					</table>
					<?php			
						/*
				...
				<div class="inboundrocket-meta-section">
					<h4 class="inboundrocket-meta-header inboundrocket-premium-tag"><?php _e('Notes', 'inbound-rocket');?></h4>
					<table class="inboundrocket-meta-table">
						<tbody>
							<tr><td>&nbsp;</td></tr>
						</tbody>
...
<?php  
        ...
        <div class="inboundrocket-contacts">
            ...
            <form id="inboundrocket-contacts-search" class="inboundrocket-contacts__search" method="GET">
                <span class="table_search">
                    ...
                    <input type="submit" name="" id="inboundrocket-search-submit" class="button" value="<?php _e('Search all contacts', 'inbound-rocket');?>">
                </span>
            </form>
        
        <?php
        $this->inboundrocket_header('Inbound Rocket '.__('Contacts','inbound-rocket'), 'inboundrocket-contacts__header');
    	?>
            ...
            <div id="bulk-edit-lists" style="display:none;">
                <h2><?php _e('Select a lead list to add to', 'inbound-rocket');?> <span class="selected-contacts-count"></span> <?php echo strtolower($inboundrocketListTable->view_label); ?></h2>
                <form id="bulk-edit-lists-form" action="" method="POST">
                    ...
                    if ( isset($inboundrocketListTable->tags) ) 
                    {
                        echo '<select name="bulk_selected_tag">';
                            foreach( $inboundrocketListTable->tags as $tag )
                                echo '<option value="' . esc_attr($tag->tag_slug) . '">' . esc_attr($tag->tag_text) . '</option>';
                        echo '</select>';
                    }
                    ?>
                    ...
                    <p class="submit">
                        <input id="bulk-edit-button" type="submit" name="submit" id="submit" class="button button-primary" value="?php _e('Add To Lead List', 'inbound-rocket');?>" />
                    </p>
                </form>
            </div>
            ...
            <?php
                ...
                if ( isset($_GET['filter_action']) || isset($_GET['filter_content']) )
                    $export_button_labels = __( 'Filtered Contacts', 'inbound-rocket' );
            ?>

            <form id="export-form" class="inboundrocket-contacts__export-form" name="export-form" method="POST">
                <input type="submit" value="<?php esc_attr_e(__('Export All ', 'inbound-rocket') . $export_button_labels ); ?>" name="export-all" id="inboundrocket-export-leads" class="button" <?php echo ( ! count($inboundrocketListTable->data) ? 'disabled' : '' ); ?> />
                <input type="submit" value="<?php esc_attr_e(__('Export Selected ', 'inbound-rocket') . $export_button_labels ); ?>" name="export-selected" id="inboundrocket-export-selected-leads" class="button" disabled />
                <input type="hidden" class="inboundrocket-selected-contacts"  name="inboundrocket_selected_contacts" value="" />
            </form>
    ...
    /**
     * Deletes all rows from ir_leads, ir_pageviews and ir_submissions for a given lead id
     *
     * @param   int
     * @return  bool
     */
    ...
    {
	    if(empty($lead_id) || !is_numeric($lead_id)) return false;
	    
        global $wpdb;
		
        $lead_hash = $wpdb->get_var( 
        	$wpdb->prepare(
        		"SELECT hashkey FROM {$wpdb->ir_leads} WHERE lead_id = %d", 
        		$lead_id
        	) 
        );
        ...
        $wpdb->query( 
        	$wpdb->prepare(
        		"UPDATE {$wpdb->ir_pageviews} SET pageview_deleted = 1 WHERE lead_hashkey = %s AND pageview_deleted = 0", 
        		$lead_hash
        	)
        );
        ...
        $wpdb->query( 
        	$wpdb->prepare(
        		"UPDATE {$wpdb->ir_submissions} SET form_deleted = 1  WHERE lead_hashkey = %s AND form_deleted = 0", 
        		$lead_hash
        	)
        );
        ...
        $wpdb->query(
        	$wpdb->prepare(
        		"UPDATE {$wpdb->ir_leads} SET lead_deleted = 1 WHERE lead_deleted = 0 AND lead_id = %d", 
        		$lead_id
        	)
        );
    ...
    function add_inboundrocket_admin_scripts ()
    {
        ...
        if ( ($pagenow == 'admin.php' && isset($_GET['page']) && strstr($_GET['page'], 'inbound-rocket')) ) 
        {
			if (INBOUNDROCKET_ENABLE_DEBUG==true) { 
				wp_register_script('inboundrocket-admin-js', INBOUNDROCKET_PATH . '/assets/js/inboundrocket-admin.js', array( 'jquery' ), FALSE, TRUE);
			} else {
				wp_register_script('inboundrocket-admin-js', INBOUNDROCKET_PATH . '/assets/js/inboundrocket-admin.min.js', array( 'jquery' ), FALSE, TRUE);
			}
...
if ( isset($_POST['export-all']) || isset($_POST['export-selected']) )
{
    ...
    {
        $search_query = $_GET['s'];
        $mysql_search_filter = $wpdb->prepare(
        	" AND ( l.lead_email LIKE '%%%s%%' OR l.lead_source LIKE '%%%s%%' ) ", 
        	$wpdb->esc_like($search_query), $wpdb->esc_like($search_query)
        );
    }
    ...
    if ( isset($_GET['filter_action']) && $_GET['filter_action'] == 'visited' )
    {
        ...
        {
            $filtered_contacts = inboundrocket_merge_filtered_contacts(
            	$wpdb->get_results(
            		$wpdb->prepare(
            			"SELECT lead_hashkey FROM $wpdb->ir_pageviews WHERE pageview_title LIKE '%%%s%%' GROUP BY lead_hashkey",
            			htmlspecialchars(urldecode($_GET['filter_content']))
            		),
					'ARRAY_A'
				),
				$filtered_contacts
    ...
    if ( isset($_GET['filter_action']) && $_GET['filter_action'] == 'submitted' )
    {
        ...
        if ( isset($_GET['filter_form']) && $_GET['filter_form'] && $_GET['filter_form'] != 'any form' )
        {
            ...
            $filter_form_query = $wpdb->prepare(
            	" AND ( form_selector_id LIKE '%%%s%%' OR form_selector_classes LIKE '%%%s%%' )",
            	$filter_form, $filter_form
            );
        }

        $q = $wpdb->prepare(
        	"SELECT lead_hashkey FROM $wpdb->ir_submissions WHERE form_page_title LIKE '%%%s%%' ", 
        	$_GET['filter_content'] != 'any page' ? htmlspecialchars(urldecode($_GET['filter_content'])) : ''
        );
