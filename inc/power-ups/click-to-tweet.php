...
@Power-up URI: https://inboundrocket.co/features/click-to-tweet/
@Power-up Description: This power-up allows you to easily create tweetable content for your readers.
@Power-up Icon: power-up-icon-click-to-tweet
@Power-up Icon Small: power-up-icon-click-to-tweet
@Power-up Sort: 1
...
class WPClickToTweet extends WPInboundRocket {
	...
	function __construct ( $activated )
	{
		...
		add_filter( 'wp_enqueue_scripts', array($this,'inboundrocket_ctt_scripts'), 10 );
	...
	public function admin_init ( )
	{
		...
		register_setting('inboundrocket_ctt_options', 'inboundrocket_ctt_options', array($this->admin, 'sanitize'));
		add_settings_section('ir_ctt_section', '', '', 'inboundrocket_ctt_options');
		add_settings_field('ir_ctt_settings', __('Click To Tweet Settings','inbound-rocket'), array($this->admin, 'ir_ctt_input_fields'), 'inboundrocket_ctt_options', 'ir_ctt_section');
	}
    ...
    function inboundrocket_ctt_scripts ()
    {
	    $this->inboundrocket_ctt_styles();
	 	...
	 	if (INBOUNDROCKET_ENABLE_DEBUG==true) { 
		 	wp_enqueue_script( "inboundrocket_ctt_script", INBOUNDROCKET_CLICK_TO_TWEET_PATH . '/js/click-to-tweet.js', array( 'jquery' ), '0.1', true );
		} else { 
			wp_enqueue_script( "inboundrocket_ctt_script", INBOUNDROCKET_CLICK_TO_TWEET_PATH . '/js/click-to-tweet.min.js', array( 'jquery' ), '0.1', true ); 
		}
    ...
    {
		if (INBOUNDROCKET_ENABLE_DEBUG==true) { 
			wp_register_style('inboundrocket_ctt_style', INBOUNDROCKET_CLICK_TO_TWEET_PATH . '/css/click-to-tweet.css', false, '0.1');
		} else { 
			wp_register_style('inboundrocket_ctt_style', INBOUNDROCKET_CLICK_TO_TWEET_PATH . '/css/click-to-tweet.min.css', false, '0.1');
		}
...
@Shorten text lenth to 100 characters.
	*/
	...
	public function tweet($text) {
	    ...
	    $short = $this->shorten(esc_attr($text), 260);
	    ...
	    if ( ! inboundrocket_check_premium_user() )   
            $str .= "<a href=\"//inboundrocket.co/?utm_source=".$_SERVER['HTTP_HOST']."&utm_medium=click-to-tweet&utm_campaign=logo\" title=\"".  __('Not using Inbound Rocket yet? - You write. We will turn them into leads','inbound-rocket')."\" target=\"_blank\" class=\"inboundrocket-ctt-branding\">&nbsp;</a>";

        $str .= "<a href=\"https://twitter.com/share?text=".urlencode($short).$handle_code."&url=".get_permalink()."\" data-text=\"".$short."\" class=\"ir-ctt-btn ctt-action\" target=\"_blank\">".__('Click To Tweet','inbound-rocket')."</a><div class=\"ir-ctt-tip\"></div></div>";
        return $str;
	}
	...
	public function tweet_feed($text) {
	    ...
	    $short = $this->shorten(esc_attr($text), 260);
	    return "<hr /><p><em>".$short."</em><br /><a href=\"https://twitter.com/share?text=".urlencode($short).$handle_code."&url=".get_permalink()."\" target=\"_blank\">".__('Click To Tweet','inbound-rocket')."</a></p><hr />";
	}
	...
	*/
	...
	private function inboundrocket_ctt_create_hashshare($options)
	{
		...
		$hashkey = sanitize_text_field($_COOKIE['ir_hash']);
		...
		if( !empty($hashkey) ) $share_array['lead_hashkey'] = $hashkey;
