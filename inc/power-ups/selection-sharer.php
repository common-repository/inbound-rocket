...
@Power-up URI: https://inboundrocket.co/features/selection-sharer/
...
class WPSelectionSharer extends WPInboundRocket {
	...
	function __construct ( $activated )
	{
		...
		add_action( 'wp_head', array($this,'inboundrocket_ss_metatag') );
	...
	public function admin_init()
	{
		...
		register_setting('inboundrocket_ss_options', 'inboundrocket_ss_options', array($this->admin, 'sanitize'));
		add_settings_section('ir_ss_section', '', '', 'inboundrocket_ss_options');
		add_settings_field('ir_ss_settings', __('Selection Sharer Settings','inbound-rocket'), array($this->admin, 'ir_ss_input_fields'), 'inboundrocket_ss_options', 'ir_ss_section');				
	}
	
	function inboundrocket_ss_metatag()
	{
		$options = isset($this->options) ? $this->options : get_option('inboundrocket_ss_options');
		if(isset($options['ir_ss_fbappid'])) echo '<meta property="fb:app_id" content="'.esc_attr($options['ir_ss_fbappid']).'">';
	}
	...
	public function inboundrocket_ss_premium()
	{
		echo '<script type="text/javascript">
			jQuery(document).ready(function($){
		    	$(\'<div class="inboundrocket-ss-branding"><a href="//inboundrocket.co/?utm_source='.$_SERVER['HTTP_HOST'].'&utm_medium=selectionsharer&utm_campaign=logo" title="'.  __("Not using Inbound Rocket yet? - You write. We will turn them into leads",'inbound-rocket') .'" class="popup-link" target="_blank">'.__('Not using Inbound Rocket yet?','inbound-rocket').'</a></div>\').insertAfter(\'body\').hide();
		});
		</script>';
	}
	...
	{	
		$options = isset($this->options) ? $this->options : get_option('inboundrocket_ss_options');
	 	...
	 	if (INBOUNDROCKET_ENABLE_DEBUG) {
		 	wp_register_style( 'inboundrocket_ss_style', INBOUNDROCKET_SELECTION_SHARER_PATH . '/css/selection-sharer.css', false, '0.1'); 
		 	wp_enqueue_script( 'inboundrocket_ss_script', INBOUNDROCKET_SELECTION_SHARER_PATH . '/js/selection-sharer.js', array( 'jquery' ), '0.1', true );
		} else {
			wp_register_style( 'inboundrocket_ss_style', INBOUNDROCKET_SELECTION_SHARER_PATH . '/css/selection-sharer.min.css', false, '0.1');
			wp_enqueue_script( 'inboundrocket_ss_script', INBOUNDROCKET_SELECTION_SHARER_PATH . '/js/selection-sharer.min.js', array( 'jquery' ), '0.1', true );
		}
	 	
	 	wp_enqueue_style( 'inboundrocket_ss_style' );
	 	
	 	if(isset($options['ir_ss_enable_frontend']) && $options['ir_ss_enable_frontend']==1) $enable = 1; else $enable = 0;
		...
		wp_localize_script( 'inboundrocket_ss_script', 'inboundrocket_ss_js', array(
			...
			'enable_frontend' 	=> $enable,
			'ir_debug'			=> INBOUNDROCKET_ENABLE_DEBUG,
			'enable_fb'			=> isset($options['ir_ss_enable_fb']) ? intval( $options['ir_ss_enable_fb'] ) : 0,
			'enable_li'			=> isset($options['ir_ss_enable_li']) ? intval( $options['ir_ss_enable_li'] ) : 0,
			'enable_tw'			=> isset($options['ir_ss_enable_tw']) ? intval( $options['ir_ss_enable_tw'] ) : 0,
			'enable_em'			=> isset($options['ir_ss_enable_em']) ? intval( $options['ir_ss_enable_em'] ) : 0,
			'email_subject' 	=> isset($options['ir_ss_email_subject']) ? esc_attr( $options['ir_ss_email_subject'] ) : '',
			'bg_color'			=> !empty($options['ir_ss_bg_color']) ? esc_attr( $options['ir_ss_bg_color'] ) : '#000000',
		));
    }
	...
	private function inboundrocket_ss_create_hashshare($options)
	{
		...
		if( isset($_COOKIE['ir_hash'])) $share_array['lead_hashkey'] = sanitize_text_field($_COOKIE['ir_hash']);
		
		$wpdb->insert($wpdb->ir_shares, $share_array);

    	$this->share_id = $wpdb->insert_id;
    	
    	if (isset($this->share_id)) {
	...
	private function inboundrocket_ss_track($post_id, $text, $type)
	{
		...
		if(empty($this->share_id)) return false;
